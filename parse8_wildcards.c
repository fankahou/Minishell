...
/*   Updated: 2025/04/05 14:19:03 by kfan             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//echo hey****"llow"*"rld"
//echo hey****"ellow**"*"rld" this is stupid, just dont put fucking stars in the quotes
...
static int filter_prefix(char *temp, char *input, int i, int index)
{
    ...
    if (prefix[0] != '\0' && index == (int)ft_strlen(input) && ft_strncmp(&input[index - ft_strlen(prefix)], prefix, ft_strlen(prefix))) // last prefix
        return(free(prefix), 1);
    else if (prefix[0] != '\0' && i == 0 && ft_strncmp(input, prefix, ft_strlen(prefix))) // first prefix
        return(free(prefix), 1);
