...
/*   Updated: 2025/03/24 16:30:28 by kfan             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// j = -1 instead of 0 to get through 25 lines, very ugly...
char *expand_envp(char *temp, t_token *token, char *new, t_clean *clean)
{
    ...
    new = ft_calloc(2, 1);
    if (!new)
        return (token->error[0] = 1, perror("ft_calloc failed\n"), NULL);
    ...
    while (token->envp[++j])
    {
        if (!ft_strncmp(temp, token->envp[j], check_sys_envp_count(token->envp[j])))
        {
            free(new);
            new = ft_strdup(&token->envp[j][i + 1]);
        }
        if (!new)
            return (ft_printf("ft_strdup failed\n"), NULL);
    }
    return (new);
}
